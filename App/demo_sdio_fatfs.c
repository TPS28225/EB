...
static void DispMenu(void)
{
	...
	printf("2 - 创建一个新文件zq_407.txt\r\n");
	printf("3 - 读zq_407.txt文件的内容\r\n");
	printf("4 - 创建目录\r\n");
	printf("5 - 删除文件和目录\r\n");
	printf("6 - 读写文件速度测试\r\n");
}
...
static void CreateNewFile(void)
{
	...
	/* 打开文件 */
	result = f_open(&file, "zq_407.txt", FA_CREATE_ALWAYS | FA_WRITE);

	/* 写一串数据 */
	result = f_write(&file, "FatFS Write Demo\r\n", 34, &bw);
	if (result == FR_OK)
	{
		printf("zq_407.txt 文件写入成功\r\n");
	}
	else
	{
		printf("zq_407.txt 文件写入失败\r\n");
	}
...
static void ReadFileData(void)
{
	...
	/* 打开文件 */
	result = f_open(&file, "zq_407.txt", FA_OPEN_EXISTING | FA_READ);
	if (result !=  FR_OK)
	{
		printf("Don't Find File : zq_407.txt\r\n");
		return;
	}
...
static void DeleteDirFile(void)
{
	...
	/* 删除文件 armfly.txt */
	result = f_unlink("zq_407.txt");
	if (result == FR_OK)
	{
		printf("删除文件 zq_407.txt 成功\r\n");
	}
	else if (result == FR_NO_FILE)
	{
		printf("没有发现文件或目录 :%s\r\n", "armfly.txt");
	...
	{
		printf("删除zq_407.txt失败(错误代码 = %d) 文件只读或目录非空\r\n", result);
	}
