...
#define 	CARDBLOCKNUM	5	
uint8_t Card_ID_Last_Time[4];
...
void RC522_IO_Init(void)
{
	...
	RCC_AHB1PeriphClockCmd(RC522_RESET_CLK, ENABLE);	
	GPIO_InitStructure.GPIO_Pin = RC522_RESET_PIN;	
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_OUT;
	GPIO_InitStructure.GPIO_Speed = GPIO_Speed_100MHz;
	GPIO_InitStructure.GPIO_OType = GPIO_OType_PP;
	GPIO_InitStructure.GPIO_PuPd  = GPIO_PuPd_UP;
	GPIO_Init(RC522_RESET_PORT, &GPIO_InitStructure);
	SPI_Configuration(RC522_SPI_DEVICE);	//目前只支持SPI3
}
...
unsigned char ReadRawRC(unsigned char Address)
{
	...
	delay_ms(1);
	RC522_ENABLE;
	SPI_WriteNBytes(RC522_SPI_DEVICE,&ucAddr,1);
	SPI_ReadNBytes(RC522_SPI_DEVICE,&ucResult,1);
	RC522_DISABLE;
	return ucResult;
}
...
{  
	...
	delay_ms(1);
	RC522_ENABLE;
	SPI_WriteNBytes(RC522_SPI_DEVICE,write_buffer,2);
	RC522_DISABLE;
}
...
u8 GET_CARDID(void)	{
	...
	{	
		...
		PcdHalt();
		INPUTDEVICE.RFID_CARDID=(Card_ID[0]<<24)+(Card_ID[1]<<16)+(Card_ID[2]<<8)+(Card_ID[0]);			
	}
	else INPUTDEVICE.RFID_CARDID=0;
	return 0;
}
...
//			if(Card_ID_Last_Time[counter_1]!=Card_ID[counter_1])break;		
//		}
//		if(counter_1==4)return 6;//排出一次检出同一张卡
		...
		status = PcdWrite(Block_Name,Card_Data);                   //写入数据
		if(status != MI_OK){
			return 5;
		}
...
u8 Read_Block(u8 Block_Name,u8* Card_Data)
{
	...
	if(MI_OK==PcdRequest(0x52, Card_Type1)){
		...
		status = PcdRead(Block_Name,Card_Data);                    //读取16字节的卡片数据
		if(status != MI_OK){
			return 5;
		}		
		PcdHalt();
...
u8 Test_CARDID(void)	
{
	...
	INPUTDEVICE.RFID_CARDID=0;
		if(MI_OK==PcdRequest(0x52, Card_Type1))
		{
			...
			Card_Data[0]=0xaa;
			status = PcdWrite(5,Card_Data);                   //写入数据
			if(status != MI_OK){
			...
			status = PcdRead(CARDBLOCKNUM,Card_Data);                    //读取16字节卡片数据
			if(status != MI_OK){
